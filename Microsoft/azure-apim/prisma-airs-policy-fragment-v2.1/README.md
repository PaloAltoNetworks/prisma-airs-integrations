...
* Claude Code: graceful blocking (200 streaming refusal so the session continues) plus automatic session/user/agent attribution
...
**Priority order:**
1. **x-claude-code-session-id header** (Claude Code) - Sent automatically by Claude Code on every request; used as the session ID with no configuration
...
> **Claude Code:** Because Claude Code emits `x-claude-code-session-id` on every call, multi-turn Claude Code sessions are grouped in AIRS automatically — no `x-session-id` required.
