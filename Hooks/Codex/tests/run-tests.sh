#!/usr/bin/env bash
# Validation harness for Codex — runs the shared fixtures through all three
...
echo "== Codex: fixtures x 3 runtimes [$MODE] =="
command -v node >/dev/null && run nodejs     node      "$HERE/../nodejs/.codex/hooks/hooks.mjs"     --vendor "$V" --event "$EV"
command -v bash >/dev/null && run bash       bash      "$HERE/../bash/.codex/hooks/airs-hooks.sh"   --vendor "$V" --event "$EV"
command -v pwsh >/dev/null && run powershell pwsh -File "$HERE/../powershell/.codex/hooks/airs-hooks.ps1" -Vendor "$V" -EventName "$EV"
