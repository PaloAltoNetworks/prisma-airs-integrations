...
> [!IMPORTANT]
> **Until a key is set, the hook passes traffic through unscanned** (loud `NOT CONFIGURED` warning on every call) so it can't brick Gemini CLI on first run — you're **unprotected** until step 2 lands. Set `AIRS_REQUIRE_CONFIG=1` to block instead while unconfigured. With a key set, any scan error fails **closed** on the input side.
