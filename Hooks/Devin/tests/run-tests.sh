#!/usr/bin/env bash
# Validation harness for Devin — runs the SAME fixtures through node + bash + powershell,
...
V="devin"; UPS_EV="UserPromptSubmit"; PRE_EV="PreToolUse"; CFG=".devin"
...
echo "== Devin [$MODE] runtimes: ${RUNTIMES[*]:-none} =="
...
case "$MODE" in
  ...
  stub)
    ...
    : # post-tool is advisory for this agent (verdict n/a); output-depth covered by the others
    ;;
  live)
