#!/usr/bin/env bash
# Validation harness for ClaudeCode — runs the shared fixtures through all three
...
echo "== ClaudeCode: fixtures x 3 runtimes [$MODE] =="
command -v node >/dev/null && run nodejs     node      "$HERE/../nodejs/.claude/hooks/hooks.mjs"     --vendor "$V" --event "$EV"
command -v bash >/dev/null && run bash       bash      "$HERE/../bash/.claude/hooks/airs-hooks.sh"   --vendor "$V" --event "$EV"
command -v pwsh >/dev/null && run powershell pwsh -File "$HERE/../powershell/.claude/hooks/airs-hooks.ps1" -Vendor "$V" -EventName "$EV"
