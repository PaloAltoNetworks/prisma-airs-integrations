...
# 🛡️ Claude Code — nodejs

**Drop-in Prisma AIRS security hooks for Claude Code, nodejs runtime.**
...
> [!NOTE]
> The wiring in `.claude/settings.json` calls the engine with a **relative** path (`.claude/hooks/hooks.mjs`), so it works as-is when the agent runs hooks from your project root — otherwise swap in an absolute path.

> [!IMPORTANT]
> **Until a key is set, the hook passes traffic through unscanned** (loud `NOT CONFIGURED` warning on every call) so it can't brick Claude Code on first run — you're **unprotected** until step 2 lands. With a key set, any scan error fails **closed** on the input side.
