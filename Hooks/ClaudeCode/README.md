...
# 🛡️ Claude Code × Prisma AIRS

**Scan every checkpoint of Claude Code — prompt, tool call, tool output, and final answer — through [Prisma AIRS](https://pan.dev/prisma-airs/).**
...
**1 · Copy the `.claude/` folder into your Claude Code project** — pick the runtime you have:
```bash
cp -r nodejs/.claude  /path/to/your/project/      # or  bash/.claude  ·  powershell/.claude
```
...
> [!IMPORTANT]
> **Configure before you rely on it.** With **no `PRISMA_AIRS_API_KEY`** set, a fresh install **passes traffic through unscanned** and prints a loud `NOT CONFIGURED` warning on every call — so copying the folder in won't brick Claude Code, but you are **not protected** until credentials land. Once a key **is** set, any AIRS error (or a half-config with a key but no profile) fails **closed** on the input side.

> [!WARNING]
> **In production, set `AIRS_REQUIRE_CONFIG=1`.** Claude Code is an env-writer (it has file/shell tools), so an injected instruction could delete this install's `.env` — a benign-looking file op AIRS won't flag — to *force* the unconfigured state and silently bypass scanning. `AIRS_REQUIRE_CONFIG=1` makes that fail **closed** (a loud DoS, not a bypass). Also deny the agent write access to the hooks dir. See [SECURITY.md](../SECURITY.md).

**3 · Start Claude Code — done.** Every checkpoint below is now scanned.
...
Each folder is self-contained (engine + wiring + `.claude/`). Shared `example.env` documents every variable; `tests/` runs the same fixtures against all three runtimes.
...
<details>
<summary><b>How enforcement works in Claude Code</b></summary>
