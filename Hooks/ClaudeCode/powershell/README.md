...
Drop-in Prisma AIRS hooks for **ClaudeCode**, powershell flavor.
...
The wiring in `.claude/settings.json` calls the engine with a **relative** path (`.claude/hooks/airs-hooks.ps1`), so it works as-is when the agent runs hooks from your project root. If not, replace it with an absolute path.
