#!/usr/bin/env bash
# Validation harness for Cursor — runs the SAME fixtures through node + bash + powershell,
...
V="cursor"; UPS_EV="beforeSubmitPrompt"; PRE_EV="beforeMCPExecution"; CFG=".cursor"
...
echo "== Cursor [$MODE] runtimes: ${RUNTIMES[*]:-none} =="
...
case "$MODE" in
  ...
  stub)
    ...
    assert_all "stub prompt   injection -> stopped"      "$HERE/fixtures/prompt-injection.json"  "$UPS_EV" ADVISE
    ...
    assert_all "stub non-string  prompt injection -> stopped"        "$HERE/fixtures/prompt-structured.json"  "$UPS_EV" ADVISE
    assert_all "stub primitive tool_input injection -> not allowed"  "$HERE/fixtures/pretool-primitive.json"  "$PRE_EV" NOT_ALLOW
    : # cursor Shell/MCP tools use the default extractor path (no per-tool field mapping)
